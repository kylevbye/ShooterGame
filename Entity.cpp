...
float Entity::getWidth() { return (float)currentFrame.w; }
float Entity::getHeight() { return (float)currentFrame.h; }
SDL_Color &Entity::getColor() { return color; }
SDL_RendererFlip Entity::getFlipSetting() { return flipSetting; }
...
void Entity::setFlipSetting(SDL_RendererFlip flipSetting) {
	this->flipSetting = flipSetting;
}
...
Entity::Entity(SDL_Texture *texture, float x, float y, SDL_Color color) 
	: texture(texture), currentFrame({}), position(x, y), activeFlag(true), centeredFlag(false), alpha(255), scaleX(1.f), scaleY(1.f), originX(0.f), originY(0.f), angleDeg(0.f), flipSetting(SDL_FLIP_NONE), color(color) {
...
Entity::Entity(Entity &&other) noexcept
	: texture(other.texture), currentFrame(other.currentFrame), position(other.getX(), other.getY()), activeFlag(other.activeFlag), centeredFlag(other.centeredFlag), alpha(other.alpha), scaleX(other.scaleX), scaleY(other.scaleY), originX(other.originX), originY(other.originY), angleDeg(other.angleDeg), flipSetting(other.flipSetting), color(other.color) {

	SDL_DestroyTexture(other.texture);
	other.texture = nullptr;
	other.currentFrame = {};
	other.position = {};
	other.activeFlag = false;
	other.centeredFlag = false;
	other.alpha = 0;
	other.scaleX = 0;
	other.scaleY = 0;
	other.originX = 0;
	other.originY = 0;
	other.angleDeg = 0;
	other.flipSetting = SDL_FLIP_NONE;
	other.color = {};
