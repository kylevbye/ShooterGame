...
std::shared_ptr<Game::Level> loadLevel(const char *levelName) {
	...
	else if (strcmp("testLevel", levelName)==0) {
		levelCode = Levels::TEST_LEVEL;
	}
	else {
...
std::shared_ptr<Game::Level> loadLevel(Levels levelCode) {
	...
	switch (levelCode) {
		...
		case Levels::GAME_OVER:
			return std::move(std::make_shared<Game::TitleLevel>());
<<<<<<< HEAD:src/AssetLoader.cpp
=======
		case Levels::TEST_LEVEL:
			return std::move(std::make_shared<Game::TestLevel>());
>>>>>>> 310f83325ba0c0a9120a95c66b1982b8d5a728f6:AssetLoader.cpp
		default:
			return std::move(std::make_shared<Game::TitleLevel>());
